...
class CuteInterpreter(object):
    ...
    def run_func(self, func_node):
        ...
        if func_node.type is TokenType.CAR:
            ...
            #작성
            #rhs2는 무조건 list라고 가정
            #Hint: get_tail()을 이용할 것!
            #get_tail() list 노드 내부의 마지막 node를 리턴함
            result_rhs1 = pop_node_from_quote_list(rhs1)
            result_rhs2 = pop_node_from_quote_list(rhs2)

            result = create_quote_node(result_rhs1)
            result.next = result_rhs2
            ...
            #작성
            result = pop_node_from_quote_list(rhs1)
            if result.type is TokenType.INT and result.next.type is TokenType.INT:
                if result.value is result.next.value:
                    return self.TRUE_NODE
            return self.FALSE_NODE

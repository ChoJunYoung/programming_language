...
class Node (object):
    ...
    def __str__(self):
        ...
        elif self.type is TokenType.APOSTROPHE:
            result = "'"+str(self.value)
        ...
        #Return self.element and string of next
            return result + " " + self.next.__str__()
...
class BasicPaser(object):
    ...
    def _create_node(self, token):
        ...
        elif token.type is CuteType.MINUS:   return Node(TokenType.MINUS,   token.lexeme)
        elif token.type is CuteType.PLUS:    return Node(TokenType.PLUS,   token.lexeme)
        elif token.type is CuteType.TRUE:    return Node(TokenType.TRUE,   token.lexeme)
        elif token.type is CuteType.FALSE:   return Node(TokenType.FALSE,   token.lexeme)
        elif token.type is CuteType.TIMES:   return Node(TokenType.TIMES,   token.lexeme)
        elif token.type is CuteType.DIV:     return Node(TokenType.DIV,   token.lexeme)
        elif token.type is CuteType.LT:      return Node(TokenType.LT,   token.lexeme)
        elif token.type is CuteType.GT:      return Node(TokenType.GT,   token.lexeme)
        elif token.type is CuteType.EQ:      return Node(TokenType.EQ,   token.lexeme)
        elif token.type is CuteType.APOSTROPHE: return Node(TokenType.APOSTROPHE,   self._parse_expr_list())
        elif token.type is CuteType.DEFINE:  return Node(TokenType.DEFINE,   token.lexeme)
        elif token.type is CuteType.LAMBDA:  return Node(TokenType.LAMBDA,   token.lexeme)
        elif token.type is CuteType.COND:    return Node(TokenType.COND,   token.lexeme)
        elif token.type is CuteType.QUOTE:   return Node(TokenType.QUOTE,   token.lexeme)
        elif token.type is CuteType.NOT:     return Node(TokenType.NOT,   token.lexeme)
        elif token.type is CuteType.CAR:     return Node(TokenType.CAR,   token.lexeme)
        elif token.type is CuteType.CDR:     return Node(TokenType.CDR,   token.lexeme)
        elif token.type is CuteType.CONS:    return Node(TokenType.CONS,   token.lexeme)
        elif token.type is CuteType.ATOM_Q:  return Node(TokenType.ATOM_Q,   token.lexeme)
        elif token.type is CuteType.NULL_Q:  return Node(TokenType.NULL_Q,   token.lexeme)
        elif token.type is CuteType.EQ_Q:    return Node(TokenType.EQ_Q,   token.lexeme)
        else:
            return None

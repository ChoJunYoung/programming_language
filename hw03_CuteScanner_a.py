...
class CuteScanner(object):
    ...
    def get_state(self, old_state, trans_char):
        if trans_char in digits+letters+'?':
            return {
                ...
                7: {k_trans8or9: 8 if k_trans8or9 is 'T' else 9 for k_trans8or9 in 'T'+'F'}
            }[old_state][trans_char]
        if old_state is 0:
            return {
                ')' : 6,    #trans_char 의 종류에 따라 old_state 를 변경해준다.
                '#' : 7,    #old_state 가 0일 경우 처음 실행이 되는 것
                '-' : 2,
                '+' : 3,
                '(' : 5,
                '<' : 12,
    ...
    def next_token(self):
        ...
        for temp_char in temp_token:
            ...
            #get_state 를 이용하여 old state 를 갱신한다.
        ...
        return result
            ...
            #temp_token 이 키워드인지 체크해서 result에 넣는다.

    def tokenize(self):
        tokens=[]
        while True:
            token_temp = self.next_token()
            if token_temp is None :
                break
            tokens.append(token_temp)
            #tokens 에 token 을 하나씩 추가한다
